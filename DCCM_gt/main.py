...
parser.add_argument('--resume', default=None, type=str, help='resume from a checkpoint')  # 检查点
parser.add_argument('--config', default='cfgs/config.yaml', help='set configuration file')  # 选择配置文件
parser.add_argument('--small_bs', default=32, type=int)
parser.add_argument('--input_size', default=96, type=int)  # 图片大小
...
def main():
	...
	# 优化器
	"""
	这段代码是使用Python标准库中的itertools模块来构建一个迭代器对象。该迭代器对象包含两个filter()函数调用的结果
	filter()函数用于根据指定的条件过滤一个可迭代对象中的元素，只保留符合条件的元素
	具体来说，这段代码的作用是将两个具有requires_grad属性的模型参数列表合并为一个可迭代对象，以便后续对这些参数进行优化更新
	首先，使用filter()函数对model.parameters()中具有requires_grad属性的参数进行过滤，只保留符合条件的参数
	然后，使用同样的方式对dim_loss.parameters()中具有requires_grad属性的参数进行过滤，并将两个过滤结果合并为一个迭代器对象
	"""
	...
	# 损失函数
	crit_graph = nn.BCELoss().cuda()  # 二元交叉熵损失
	crit_label = WeightedBCE().cuda()  # 加权二元交叉熵损失
	crit_c = nn.CrossEntropyLoss().cuda()  # 交叉熵损失
